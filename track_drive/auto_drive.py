...
class AutoDrive:
    ...
    동작:
        ...
        4. 노란색 점선이 끊기면 이전 직선 모델로 예측
        5. 예측도 불가능하면 마지막 조향각 유지
    """
    ...
    def __init__(self, logger=None, show_debug=True):
        ...
        self.target_center_offset_px = 0.0
    ...
    def detect_yellow_line(self, image):
        ...
        target_x = (
            width * self.target_center_x_ratio
            + self.target_center_offset_px
        )

        target_x = self.clamp(target_x, 0.0, float(width - 1))
    ...
    def show_debug_view(self, image):
        ...
        if self.last_target_x is not None:
            ...
            target_x = int(self.clamp(target_x, 0, width - 1))
